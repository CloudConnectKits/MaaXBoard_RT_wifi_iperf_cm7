...
#define EXT_AP_SSID       "2WIRE872"
#define EXT_AP_PASSPHRASE "LoonyLizaard!"

#ifndef IPERF_SERVER_ADDRESS
#define IPERF_SERVER_ADDRESS "192.168.1.15"
#endif
...
{
    PRINTF("MaaXBoard USER LED is ON after valid menu choice\r\n");
    PRINTF("Press <spacebar> to exit an active mode...\r\n");
    printSeparator();
...
{
    GPIO_PinWrite(BOARD_USER_LED_GPIO, BOARD_USER_LED_GPIO_PIN, 0U);   // USER_LED turned OFF
	if (ch == '\r' || ch == ' ')
    {
        menuSeparator();
        return;
    ...
    for (int i = 0; menuItems[i].func != NULL; i++)
    {
        ...
        {
            PRINTF("Key '%c': %s\r\n", ch, menuItems[i].text);
            GPIO_PinWrite(BOARD_USER_LED_GPIO, BOARD_USER_LED_GPIO_PIN, 1U);   // USER_LED turned ON (if valid menu selection)
            menuItems[i].func();
            return;
        }
    }
...
int main(void)
{
    ...
    printSeparator();
    PRINTF("MaaXBoard RT - wifi iperf demo\r\n");
    printSeparator();
